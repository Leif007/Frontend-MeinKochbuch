...
function addRecipe() {
  ...
  recipes.value.push({...newRecipe}); // Use .value to modify the recipes array
  newRecipe = {
    name: "",
    picture: "",
    preparationTime: "",
...
let dishTypes = computed(() => {
  let types = recipes.map(recipe => recipe.dishType);
...
let filteredRecipes = computed(() => {
  return recipes.filter(recipe => {
    return (selectedDishType.value === '' || recipe.dishType === selectedDishType.value) &&
...
watch(recipes, () => {
  dishTypes.value = [...new Set(recipes.map(recipe => recipe.dishType))];
  mealTimes.value = [...new Set(recipes.map(recipe => recipe.mealTime))];
  dietTypes.value = [...new Set(recipes.map(recipe => recipe.dietType))];
...
<template>
  <div class="recipeheader">
    ...
    <select id="dishType" v-model="selectedDishType">
    ...
    <div class="addrecipe">
      <form @submit.prevent="addRecipe" v-if="showForm">
        ...
        <label>Dish Type: <input v-model="newRecipe.dishType" required /></label>
